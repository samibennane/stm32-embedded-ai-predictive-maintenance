...
#if !defined(AI_MACHINE_FAILURES_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_MACHINE_FAILURES_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_MACHINE_FAILURES_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_MACHINE_FAILURES_IN_NUM] = {
...
#if !defined(AI_MACHINE_FAILURES_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_MACHINE_FAILURES_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_MACHINE_FAILURES_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_MACHINE_FAILURES_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_MACHINE_FAILURES_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle machine_failures = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_machine_failures_create_and_init(&machine_failures, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_machine_failures_create_and_init");
    return -1;
  }

  ai_input = ai_machine_failures_inputs_get(machine_failures, NULL);
  ai_output = ai_machine_failures_outputs_get(machine_failures, NULL);

#if defined(AI_MACHINE_FAILURES_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_MACHINE_FAILURES_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_MACHINE_FAILURES_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_machine_failures_run(machine_failures, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_machine_failures_get_error(machine_failures),
        "ai_machine_failures_run");
    return -1;
  }
